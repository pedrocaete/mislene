...
class RuaField extends TextField
{
    protected $type = 'Rua';
    ...
    protected function getOptions()
    {
        ...
        $query->select('DISTINCT ' . $db->quoteName('rua', 'value'))
            ->select($db->quoteName('rua', 'text'))
            ->from($db->quoteName('#__clientmanager_clients'))
            ->where($db->quoteName('rua') . ' IS NOT NULL')
            ->where($db->quoteName('rua') . ' != ' . $db->quote(''))
            ->order($db->quoteName('rua') . ' ASC');
